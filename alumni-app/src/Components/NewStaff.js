...
import { staffregisterRoute } from '../utils/ApiRoutes';
...
const NewStaff = () => {
  ...
  const [user, setUser] = useState({
    name: '', profilePhoto: '', birth: '',join:'', email: '', contact: '', password: '', confirmpassword: ''
  });
  ...
  const submitForm = async (e) => {
    ...
    const { name, profilePhoto, birth,join, email, contact, password, confirmpassword } = user;
    ...
    if (name === "") {
        ...
        const res = await fetch(staffregisterRoute, {
          method: 'POST',
          headers: {
    ...
    <>
      <div className='container border border-dark p-2 mt-2'>
        ...
        <form method='POST'>
          ...
          <input className="form-control w-50" type="file" id="formFile" name='profilePhoto' value={user.profilePhoto} onChange={handleInputs}></input>
          ...
          <input id="startDate" className="form-control w-50" type="date" name='join' value={user.join} onChange={handleInputs} />
